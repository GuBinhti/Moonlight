...
state = {
    ...
    "speed_factor": 1.0,
    "day_length_in_real_seconds": 86400,  # 24 h default
    "hex_color": "FF0000",
...
def serve_plot(filename): return send_from_directory(OUTPUT_DIR, filename)
...
def plot_phase():  plot_moon_phase_angle(state["moon_schedule"]);  return jsonify({"image": _save_plot(f"phase_angle_{_timestamp()}.png")})
@app.route("/plot-rise-set")
def plot_rs():     plot_moon_schedule_times(state["moon_schedule"]); return jsonify({"image": _save_plot(f"rise_set_{_timestamp()}.png")})
@app.route("/plot-phases")
def plot_phs():    plot_moon_schedule_phases(state["moon_schedule"]); return jsonify({"image": _save_plot(f"phases_{_timestamp()}.png")})
@app.route("/plot-altitude", methods=["POST"])
def plot_alt():
    idx = int((request.get_json() or {}).get("day", 0))
    if 0 <= idx < len(state["moon_schedule"]):
        plot_hourly_altitude(state["moon_schedule"][idx], state["cycle_start_date"], 30)
