...
def simulation_loop(
        schedule,
        cycle_start_date: datetime.datetime,
        user_cycle_length: int,
        update_interval_minutes: float,
        speed_factor: float,
        day_length_in_real_seconds: float,
        hex_color: str,
        feed_start_time: str,
        feed_end_time: str,
        stop_event: threading.Event,
        shared_state: dict | None = None,  # kept positional order for API stability
        sim_start_time: datetime.time | None = None,  # new
):
    """
    ...
    if sim_start_time is None:
        ...
        simulation_time = datetime.datetime.combine(cycle_start_date.date(), sim_start_time) # new
...
def handle_command(cmd, arg, stop_event, state):
    if cmd == 'pt':
        ...
        if not state['simulation_started']:
            # === CHANGE 🟢 3/3 – prompt for arbitrary HH:MM ==================
            start_time_input = input(
                "Enter simulation start time in HH:MM (24 h) or press Enter for now: "
            ).strip()
            sim_start_time = None
            if start_time_input:
                try:
                    sim_start_time = datetime.datetime.strptime(start_time_input, "%H:%M").time()
                except ValueError:
                    print("[Main] Invalid time – starting at current clock time.")

            print("[Main] Launching simulation…")
            state['simulation_started'] = True
            state['sim_start_time'] = sim_start_time
            ...
            sim_thread = threading.Thread(
                ...
                kwargs={'sim_start_time': sim_start_time},
                daemon=True,
            )
            sim_thread.start()
