...
def plot_moon_schedule_times(schedule):
    ...
    for entry in schedule:
        day_label = entry['day']   # already 1…N
...
def simulation_loop(
    ...
    while not stop_event.is_set():
        # ─── figure out if it's day or night now ───
        is_day = SUNRISE_HOUR <= simulation_time.hour < SUNSET_HOUR

        # ─── detect transitions and bump counters ───
        ...
        if is_day:
            altitude_deg = 90.0
            print(
                f"[Sim {simulation_time:%Y-%m-%d %H:%M}] "
                f"Day {day_count} – Sun is out (alt={altitude_deg:.1f}°)."
            )
        else:
            altitude_deg = calculate_current_altitude(
                entry, simulation_time, cycle_start_date
            )
            if altitude_deg > 0:
                print(
                    f"[Sim {simulation_time:%Y-%m-%d %H:%M}] "
                ...
                print(
                    f"[Sim {simulation_time:%Y-%m-%d %H:%M}] "
                    f"Night {night_count} – Moon not visible (alt=0)."
                )
        ...
        if independent_timer:
            if simulation_time.strftime('%H:%M') == drop_countdown and feeder_thread is None:
                print("Independent Timer Started")
                feeder_thread = threading.Thread(target=drop_alarm, daemon=True)
                feeder_thread.start()
            if simulation_time.strftime('%H:%M') == end_feed_countdown and feeder_thread is None:
