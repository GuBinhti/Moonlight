...
def calculate_moonrise_times(target_cycle_length):
    ...
    for day in range(new_total_days):
        ...
        irr_fraction = moon_irradiance_fraction(day, new_total_days)
        ...
        results.append({
            ...
            'irradiance_fraction': irr_fraction
        })

    return results

def moon_irradiance_fraction(day, cycle_length):
    ...
    return 1.0 - abs(1.0 - 2.0 * y / cycle_length)
...
def plot_moon_irradiance(schedule):

    days = [entry['day'] for entry in schedule]
    irr  = [entry['irradiance_fraction'] for entry in schedule]

    plt.figure(figsize=(8,4))
    plt.plot(days, irr, marker='o', color='blue')
    plt.title("Moon Irradiance Over Lunar Month")
    plt.xlabel("Day in Lunar Month")
    plt.ylabel("Irradiance Fraction")
    plt.ylim(0, 1.05)  # small padding above 1
...
def simulation_loop(schedule, cycle_start_date, user_cycle_length, update_interval_minutes, speed_factor, day_length_in_real_seconds, stop_event):
    ...
    while not stop_event.is_set():  # loop until told to stop
        ...
        if current_entry is not None:
            ...
            irr_fraction = current_entry.get('irradiance_fraction', 0.0)
...
def handle_command(cmd, arg, schedule, cycle_start_date, stop_event):
    if cmd == 'pt':
        ...
        print("Plotting Moon Irradiances...")
        plot_moon_irradiance(schedule)
