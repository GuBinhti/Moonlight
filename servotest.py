...
def set_servo_angle(pwm, angle):
    angle = max(0, min(angle, 180))  # Ensure angle is between 0 and 180
    duty_cycle = ((angle / 180) * 10) + 2.5  # Maps angle to 2.5%-12.5% duty cycle

    print(f"Setting Servo Angle: {angle}° | Duty Cycle: {duty_cycle:.2f}%")
...
def moon_placement_azimuth(year, month, day, hour, minute, second, pwm):
    ...
    while True:
        ...
        if local_time >= next_print_time:
            ...
            servo_angle = max(0, min(180, (zenith_angle / 90) * 180))  # Proper mapping
